...
Blackbaud (and similar) feeds often omit `DTEND`, or copy `DTSTART` into `DTEND`, so calendar apps show a zero-length event. Timed games then get a **guessed 1-hour window** (`DTSTART` + 60 minutes). The event `DESCRIPTION` (calendar Notes) includes `End time is a guess.` Real end times and all-day dates are left alone.
